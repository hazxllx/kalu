  ...
  return (
    <>
        ...
        <Card className="overflow-hidden">
          <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-brand-border">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-heading font-semibold text-brand-ink">Pending Resident Verifications</h3>
                <p className="text-xs text-brand-gray">{PENDING_VERIFICATIONS.length} residents awaiting identity review</p>
              </div>
            </div>
            <Link to="/app/bhw/verifications" className="flex items-center gap-1 text-sm font-medium text-brand-blue hover:underline">
              View All <ChevronRight className="w-4 h-4" />
            </Link>
          </div>
          <div className="divide-y divide-brand-border">
            {PENDING_VERIFICATIONS.map((r) => (
              <div key={r.ref} className="flex items-center justify-between px-6 py-3 hover:bg-brand-bg/50 transition-colors">
                <div className="flex items-center gap-3">
                ...
                <Link to="/app/bhw/verification/review" className="text-sm font-medium text-brand-blue hover:underline">Review</Link>
              </div>
            ))}
          </div>
        </Card>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <Card className="p-6">
          <h3 className="font-semibold text-brand-ink mb-4">Today's Follow-ups</h3>
          <div className="space-y-3">
            {followUps.filter((f) => f.status === "Today").map((f) => (
              <div key={f.resident} className="flex items-center justify-between border border-brand-border rounded-btn px-4 py-3">
                <div>
        ...
        <Card className="p-6">
          <h3 className="font-semibold text-brand-ink mb-4">High Risk Residents</h3>
          <div className="space-y-3">
            {residents.filter((r) => r.risk === "High").map((r) => (
              <div key={r.id} className="flex items-center justify-between border border-brand-border rounded-btn px-4 py-3">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-brand-light text-brand-blue flex items-center justify-center text-sm font-semibold">{r.name.split(" ").map(n=>n[0]).slice(0,2).join("")}</div>
                  <div>
