...
import {
  ...
  MapPin, Calendar, ChevronDown, Shield, Camera, FileText, Edit3, ShieldCheck,
} from "lucide-react";
...
const BARANGAYS = ["San Jose", "San Isidro", "Old San Roque", "Sta. Cruz", "Bagong Silang", "San Antonio", "San Vicente", "Santo Nio"];
...
const STEPS_META = [
  ...
  { num: 2, title: "Contact Information", subtitle: "How can we reach you?", icon: Phone },
  { num: 3, title: "Create Your Account", subtitle: "Set up your login credentials.", icon: Shield },
  { num: 4, title: "Face Verification", subtitle: "Capture or upload a photo for identity verification.", icon: Camera },
  { num: 5, title: "Review Your Information", subtitle: "Please verify all details before submitting.", icon: FileText },
  { num: 6, title: "Submit Registration", subtitle: "Confirm and submit your registration.", icon: ShieldCheck },
];
...
export default function Register() {
  ...
  const [form, setForm] = useState({
    firstName: "", middleName: "", lastName: "", suffix: "", dob: "", sex: "", civilStatus: "",
    mobile: "", province: "Camarines Sur", municipality: "Pili", barangay: "", sitio: "", street: "",
    houseNo: "", landmark: "", occupation: "",
  ...
  const next = () => { if (validateStep(step)) setStep((p) => Math.min(p + 1, 6)); };
    ...
    <Shell>
      <div className="w-full max-w-2xl">
        <div className="text-center mb-6">
          <img src={LOGO_URL} alt="KALUSAGAP" className="h-10 w-auto mx-auto mb-6" />
          ...
          <StepWrapper step={step}>
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-body font-medium text-brand-gray">Step {step} of 6</span>
              </div>
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <TextField label="Birth Date" type="date" icon={Calendar} value={form.dob} onChange={(e) => setForm({ ...form, dob: e.target.value })} error={errors.dob} required />
            ...
            {step === 5 && (
              <div className="space-y-4">
                ...
                <ReviewSection title="Account Details" onEdit={() => goTo(3)} items={[
                  ["Email Address", form.email],
                ]} />
                <ReviewSection title="Face Verification" onEdit={() => goTo(4)} items={[
                  ["Photo", form.facePhoto ? "Captured — pending review" : "Not captured"],
                ]} />
              </div>
            )}
            ...
            {step === 6 && (
              <div className="text-center py-6">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", delay: 0.1 }}
                  className="w-16 h-16 rounded-full bg-brand-blue/10 flex items-center justify-center mx-auto">
                  <ShieldCheck className="w-8 h-8 text-brand-blue" strokeWidth={1.8} />
                </motion.div>
                <h2 className="mt-5 text-lg font-heading font-semibold text-brand-ink">Ready to Submit</h2>
                <p className="mt-2 text-sm text-brand-gray max-w-md mx-auto">
                  Please confirm that all information provided is correct. Your assigned Barangay Health Worker will review your registration before your account receives full access.
                </p>
                <div className="mt-5 bg-brand-bg rounded-card p-4 text-left">
                  <p className="text-xs text-brand-gray">
                    By submitting, you confirm that the information provided is true and accurate. You will receive a limited resident account while your registration is under review.
                  </p>
                </div>
              </div>
            )}
