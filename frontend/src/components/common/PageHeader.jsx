  ...
  return (
    <div className="mb-8 rounded-2xl border border-slate-200 bg-white px-5 py-5 shadow-card dark:border-border dark:bg-card md:px-6">
      <div className="flex items-start justify-between gap-3">
        <nav className="flex min-w-0 flex-wrap items-center gap-1.5 text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">
          {crumbs.map((c, i) => (
      ...
      <div className="mt-3 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="min-w-0 flex-1">
          <h1 className="text-2xl font-semibold tracking-tight text-slate-900 dark:text-foreground md:text-3xl">{title}</h1>
