...
/**
 * Session-scoped resident registry (local in-session state only).
 *
 * There is NO demo data: the resident list starts empty. This module remembers
 * residents created during the current browser session (kept in
 * `sessionStorage` so new residents survive a page refresh within the same tab)
 * until the corresponding backend endpoint persists them to the database. The
 * record shape is the SAME shape the resident directory already renders:
 *
 *   { id, name, age, gender, risk, barangay, status, program }
 *
 * plus the extra profile fields captured by the Add Resident form (DOB, contact
 * number, address, civil status) that are displayed by the resident detail
 * view.
 */

const STORAGE_KEY = "kalusagap.residents.session.v2";
...
/**
 * Risk classification for newly registered residents.
 *
 * Resident risk is NOT a manual entry — it is derived automatically when the
 * record is created. This is a transparent, deterministic heuristic; a clinical
 * ruleset can replace it later.
 *
