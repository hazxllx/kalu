...
import { Search, Download, SlidersHorizontal, Eye, Plus } from "lucide-react";
      ...
      <PageHeader crumbs={["Home", "Residents"]} title="Resident Directory" subtitle="Search, filter, and manage residents in your area."
        action={
          <div className="flex items-center gap-3">
            <button className="hidden sm:flex items-center gap-2 border border-brand-border bg-white px-4 py-2.5 rounded-btn text-sm font-medium text-brand-ink hover:border-brand-blue transition-colors"><Download className="w-4 h-4" /> Export</button>
            <button className="flex items-center gap-2 bg-brand-blue text-white px-4 py-2.5 rounded-btn text-sm font-medium hover:bg-brand-dark transition-colors"><Plus className="w-4 h-4" /> Add Resident</button>
          </div>
        } />
